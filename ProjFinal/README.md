...
## Conclusão
Observando os resultados finais, é possível concluir que o código utilizando a biblioteca OpenMP conseguiu mais performance quando comparado com o código utilizando a biblioteca Pthread. Isso pode ter acontecido por alguns motivos, como a própria diferença de biblioteca usada em si, pois a biblioteca Pthread possui mais overhead, ou até algumas decisões de otimização diferentes dentro dos códigos. Também é importante notar que o código utilizando OpenMP ficou mais performático quanto mais threads foram adicionadas no seu processamento, enquanto o código de Pthread ficou com um tempo de execução igual ou pior quando se adicionava mais threads, então o código OpenMP também ficou mais paralelizável, possívelmente por causa do overhead serial do outro código.  
